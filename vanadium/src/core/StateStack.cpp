...
State *StateStack::top() const noexcept
{
...
State *StateStack::get(VNsize index) const noexcept
{
    ...
    {
        VAN_ENGINE_ERROR("Unable to retrieve state with index {}. Total states: {}", index, this->states.size());
        return nullptr;
    }
    return this->states.back();
}

VNsize StateStack::size() const noexcept
{
