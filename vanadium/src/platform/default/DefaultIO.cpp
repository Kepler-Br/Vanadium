...
#else
#warning \
    "DefaultIO in current C++(< C++17) standard has no implementations for many methods! Not implemented methods will throw exceptions."
bool DefaultIO::fail() { return this->isFail; }
...
void DefaultIO::writeFile(const std::string &path, void *data, VNsize dataSize,
                          bool overwrite) {
  std::ofstream file;
...
std::vector<std::string> DefaultIO::listDirectory(const std::string &path) {
  throw std::runtime_error(
      "std::filesystem is not supported in current C++ standard.");
}
