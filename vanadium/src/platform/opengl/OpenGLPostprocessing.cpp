...
OpenGLPostprocessing::OpenGLPostprocessing(const Ref<Shader> &shader, VNuint width, VNuint height) :
        shader(shader)
{
...
OpenGLPostprocessing::OpenGLPostprocessing(const Ref<Shader> &shader, const Framebuffer::Specification &specification) :
    shader(shader)
{
...
void OpenGLPostprocessing::unbind() noexcept
{
    ...
    glViewport(this->viewportSize.x, this->viewportSize.y, this->viewportSize.z, this->viewportSize.w);
}
...
void OpenGLPostprocessing::bindShader() noexcept
{
    this->shader->bind();
}

void OpenGLPostprocessing::unbindShader() noexcept
{
    this->shader->unbind();
}
