...
// OpenGLShader::OpenGLShader(const std::string &shaderName, const std::string
// &vertex, const std::string &pixel)
//{
...
OpenGLShader::OpenGLShader(const ShaderMap &shaderSources,
...
void OpenGLShader::unbind() const noexcept { glUseProgram(0); }
...
VNint OpenGLShader::getGlobalId(const std::string &name) noexcept {
  ...
  if (uniformLocation == -1) {
    VAN_ENGINE_ERROR(
        "Cannot get uniform location with name \"{}\" from shader \"{}\".",
        name, this->name);
...
void *OpenGLShader::getRaw() const noexcept { return (void *)&this->pointer; }
...
void OpenGLShader::setGlobalFloat2(const std::string &name,
...
void OpenGLShader::setGlobalFloat3(const std::string &name,
...
void OpenGLShader::setGlobalFloat4(const std::string &name,
...
void OpenGLShader::setGlobalMat2(const std::string &name,
...
void OpenGLShader::setGlobalMat3(const std::string &name,
...
void OpenGLShader::setGlobalMat4(const std::string &name,
...
const std::string &OpenGLShader::getName() const noexcept { return this->name; }
...
void OpenGLShader::compile(const ShaderMap &shaderSources) {
  ...
  for (const auto &shader : shaderSources) {
    ...
    //        VAN_ENGINE_TRACE("Compiling \"{}\" shader.",
    //        Shader::typeToString(shader.first));
...
GLuint OpenGLShader::compileShaderProgram(const std::string &source,
                                          GLenum shaderType) {
...
void OpenGLShader::destroyShaderPrograms(
    const std::vector<GLuint> &shaderProgramIDs) const {
...
bool OpenGLShader::operator!() const noexcept { return (this->pointer == 0); }
...
GLenum OpenGLShader::shaderTypeToOpenGLType(Shader::Type type) {
  switch (type) {
    ...
    default:
      VAN_ENGINE_ASSERT(
          false, "OpenGLShader::shaderTypeToOpenGLType: unknown shader type.");
      return 0;
  }
}
