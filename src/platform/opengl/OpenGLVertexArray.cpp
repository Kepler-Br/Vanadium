...
GLenum OpenGLVertexArray::dataTypeToOpenGLEnum(DataTypes type) {
  switch (type) {
    ...
    default:
      VAN_ENGINE_ASSERT(
          false, "OpenGLVertexArray::dataTypeToOpenGLEnum: unknown data type!");
      return GL_INT;
  }
}
...
void OpenGLVertexArray::unbind() const noexcept { glBindVertexArray(0); }
    ...
    switch (element.type) {
      ...
      case DataTypes::Float4: {
        ...
        glVertexAttribPointer(
            (GLuint)this->vertexBufferIndex, (GLint)element.getComponentCount(),
      ...
      default:
        VAN_ENGINE_ASSERT(
            false, "OpenGLVertexArray::addVertexBuffer: Unknown data type!");
    }
  }
...
void OpenGLVertexArray::setIndexBuffer(
    const Ref<IndexBuffer> &indexBuffer) noexcept {
...
const Ref<IndexBuffer> &OpenGLVertexArray::getIndexBuffer() const noexcept {
  if (!this->indexBuffer)
    VAN_ENGINE_ERROR(
        "OpenGLVertexArray::getIndexBuffer: indexBuffer is nullptr!");
...
bool OpenGLVertexArray::operator!() const noexcept {
  return (this->pointer == 0) || (this->indexBuffer == nullptr) ||
         (this->vertexBuffers.empty());
}
