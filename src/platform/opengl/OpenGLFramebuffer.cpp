...
void OpenGLFramebuffer::attachColorTexture(VNsize id, int samples,
                                           GLenum format, VNsize width,
                                           VNsize height, int index) {
...
void OpenGLFramebuffer::attachDepthTexture(uint32_t id, GLsizei samples,
...
bool OpenGLFramebuffer::isDepthFormat(Framebuffer::TextureFormat format) {
  if (format == Framebuffer::TextureFormat::DepthStencil) return true;
  return false;
}
...
void OpenGLFramebuffer::create() {
  ...
  if (!this->colorAttachmentSpecifications.empty()) {
    ...
    OpenGLFramebuffer::createTextures(multisample,
                                      this->colorAttachments.data(),
                                      this->colorAttachments.size());
    ...
    for (size_t i = 0; i < this->colorAttachments.size(); i++) {
      ...
      switch (this->colorAttachmentSpecifications[i].textureFormat) {
        case Framebuffer::TextureFormat::RGBA8:
          OpenGLFramebuffer::attachColorTexture(
              this->colorAttachments[i], this->specification.samples, GL_RGBA8,
              this->specification.width, this->specification.height, i);
          break;
        default:
          VAN_ENGINE_ASSERT(false,
                            "OpenGLFramebuffer::create "
                            "Framebuffer::TextureFormat is not RGBA8.");
          break;
      }
    }
  }

  if (this->depthAttachmentSpecification.textureFormat !=
      Framebuffer::TextureFormat::None) {
    ...
    switch (this->depthAttachmentSpecification.textureFormat) {
      case Framebuffer::TextureFormat::DepthStencil:
        OpenGLFramebuffer::attachDepthTexture(
            this->depthAttachment, this->specification.samples,
      ...
      default:
        VAN_ENGINE_ASSERT(false,
                          "OpenGLFramebuffer::create "
                          "Framebuffer::TextureFormat is not DepthStencil.");
        break;
    }
  }
  ...
  if (!this->colorAttachments.empty()) {
    ...
    glDrawBuffers(this->colorAttachments.size() > maxColorAttachments
                      ? maxColorAttachments
                      : this->colorAttachments.size(),
                  buffers);
  } else {
...
void OpenGLFramebuffer::destroy() {
  ...
  glDeleteTextures(this->colorAttachments.size(),
                   this->colorAttachments.data());
...
void *OpenGLFramebuffer::getColorAttachment(VNsize index) const noexcept {
  if (index >= this->colorAttachments.size()) {
    VAN_ENGINE_ERROR(
        "OpenGLFramebuffer::getColorAttachment: requested not existing color "
        "attachment texture index: {}. Total color attachments: {}.",
        index, this->colorAttachments.size());
    return nullptr;
  }
