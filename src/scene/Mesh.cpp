...
Ref<Mesh> MeshFactory::wireframePlane(glm::vec2 bottomLeft,
  ...
  Ref<VertexBuffer> vbo = VertexBufferFactory::create(
      vboData.data(), vboData.size() * sizeof(VNfloat));
...
Ref<Mesh> MeshFactory::plane(glm::vec3 bottomLeft, glm::vec3 bottomRight,
  ...
  Ref<VertexBuffer> vbo = VertexBufferFactory::create(
      &vboData[0], vboData.size() * sizeof(VNfloat));
...
Ref<Mesh> MeshFactory::unitCube(VNfloat multiplication) {
  ...
  Ref<VertexBuffer> vbo = VertexBufferFactory::create(
      &vertices[0], vertices.size() * sizeof(VNfloat));
  vbo->setLayout({{DataTypes::Float4, "a_Position"}});

  Ref<VertexBuffer> uvBO =
      VertexBufferFactory::create(&uvs[0], uvs.size() * sizeof(VNfloat));
  uvBO->setLayout({{DataTypes::Float2, "a_UV"}});

  Ref<VertexBuffer> normalBO = VertexBufferFactory::create(
      &normals[0], normals.size() * sizeof(VNfloat));
...
Ref<Mesh> MeshFactory::unitCircle(VNint verticesCount, VNfloat multiplication) {
  ...
  for (VNint i = 0; i < verticesCount; i++) {
    ...
    vertices.push_back(std::cos(angle) * multiplication * lineDivider * 1.2f /
                       lineDivider);
    vertices.push_back(std::sin(angle) * multiplication * lineDivider * 1.2f /
                       lineDivider);
    vertices.push_back(std::cos(angle) * multiplication * lineDivider * 1.5f /
                       lineDivider);
    vertices.push_back(std::sin(angle) * multiplication * lineDivider * 1.5f /
                       lineDivider);
...
Ref<Mesh> MeshFactory::fromVertices(VNfloat *vertices, VNsize size,
  ...
  Ref<VertexBuffer> vbo =
      VertexBufferFactory::create(vertices, size * sizeof(VNfloat));
...
Ref<Mesh> MeshFactory::fromVerticesIndices(
    VNfloat *vertices, VNsize vertexCount, VNuint *indices, VNsize indicesCount,
    Mesh::PrimitiveType targetPrimitiveType) {
  Ref<VertexBuffer> vbo =
      VertexBufferFactory::create(vertices, vertexCount * sizeof(VNfloat));
  vbo->setLayout({{DataTypes::Float2, "a_Position"}});
