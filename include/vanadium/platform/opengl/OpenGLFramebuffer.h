...
class OpenGLFramebuffer : public Framebuffer {
 private:
  ...
  void attachColorTexture(VNsize id, int samples, GLenum format, VNsize width,
                          VNsize height, int index);
  void attachDepthTexture(uint32_t id, GLsizei samples, GLenum format,
                          GLenum attachmentType, GLsizei width, GLsizei height);
 ...
 public:
  ...
  [[nodiscard]] VNuint getWidth() const noexcept override;
  [[nodiscard]] VNuint getHeight() const noexcept override;
  [[nodiscard]] glm::vec2 getGeometry() const noexcept override;
  bool operator!() const noexcept override;
  [[nodiscard]] void *getColorAttachment(VNsize index) const noexcept override;
  [[nodiscard]] void *getDepthAttachment() const noexcept override;
};
